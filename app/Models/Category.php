...
use Modules\Woocommerce\Http\Controllers\WoocommerceCategorySyncController;
...
class Category extends Model
{
    ...
    {
        parent::boot();
        if (isset(auth()->user()->id) && ! empty(json_decode(auth()->user()->business->woocommerce_api_settings, true)['enable_woo_auto_sync'])) {
            static::created(function ($category) {
                self::syncCategory($category);
            });
    ...
    protected static function syncCategory($category)
    {
        if (! empty(auth()->user())) {
            (new WoocommerceCategorySyncController)->syncCategories($category['business_id'], $category['created_by']);
        }
    }

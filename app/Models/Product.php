...
class Product extends Model
{
    ...
    public function getImageUrlAttribute()
    {
        ...
        if (! empty(request()->header('X-Tenant'))) {
            if (! empty($this->image)) {
                $image_url = remote_asset('/uploads_new/img/'.rawurlencode($this->image));
            } else {
                $image_url = asset('/img/default.png');
            }
        }
    ...
    public static function forDropdown($business_id, $append_all = false)
    {
        $sold_product = TransactionSellLine::join(
            'products AS p',
            ...
            ->pluck('product_name', 'variation_id');

        if ($append_all) {
            $sold_product->prepend(__('lang_v1.all_products'), '');
        }
